% FINALISEJOINTSTRUCT Create astructure of complete lower limb joints
% parameters based on the results of the lower limb morphological analyses
% and the joint definitions specified as inputs. 
%
% jointStruct = finalizeJointStruct(osimModel, JCS, jointDefs, jointParamFile)
%
% Inputs:
%
%   JCS - a MATLAB structure created using the function
%       processTriGeomBoneSet(). This structure includes as fields the
...
%
% Outputs:
%   jointStruct - a MATLAB structure with all the information required to
%                 create OpenSim joints. This differs from the initial JCS
%                 because that initial structure will have some missing
%                 fields, i.e. the morphological analysis does not provide
%                 all the information for building all the joints.
%
...
function jointStruct = finalizeJointStruct(JCS, joint_defs, jointParamFile)
