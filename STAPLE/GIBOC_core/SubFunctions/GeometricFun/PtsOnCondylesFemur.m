function [ PtsCondyle_end, PtsKeptID ] = PtsOnCondylesFemur( PtsCondyle_0 , Pts_Epiphysis, CutAngle, ellip_shrink_fact, ellip_dilat_fact ) %ellip_shrink_fact
    % Find points on condyles from a first 2D ellipse. Fit on
    ...
    %
    % :param PtsCondyle_0: A points cloud of points located on the one of the femur condyles.
    % :type PtsCondyle_0: (n x 3) floats matrix
    % :param Pts_Epiphysis: The points cloud the one half (medial or lateral) of the distal femur epiphysis.
    % :type Pts_Epiphysis: (n x 3) floats matrix
    % :param CutAngle: The points cloud the one half (medial or lateral) of the distal femur epiphysis.
    % :type CutAngle: angle in degree
    % :param ellip_shrink_fact: A ratio by which to reduce the fitted ellipse size.
    % :type ellip_shrink_fact: floats in [0,1]
    % :param ellip_dilat_fact: A ratio by which to increase the fitted ellipse size.
    % :type ellip_dilat_fact: floats in [0,1]
    % :return: PtsCondyle_end: A final extended points cloud of one of the femur condyles.
    % :rtype: (m x 3) floats matrix
    % :return:PtsKeptID: The index of the points in PtsCondyle_end in Pts_Epiphysis
    % :rtype: (m x 1) integer matrix
    ...
    ellipse_x_r     = Elps.X0 + ellip_shrink_fact*Elps.a*cos( theta_r );
    ellipse_y_r     = Elps.Y0 + ellip_shrink_fact*Elps.b*sin( theta_r );
    ...
    % Points must be outside of the reduced ellipse and inside the Convexhull
