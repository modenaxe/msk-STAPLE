...
% COMPUTEMASSPROPERTIES_MIRTICH1996
...
function MassProps = computeMassProperties_Mirtich1996(v, f)
	...
	% implementation was taken from Eberly, D., 2003. Polyhedral mass 
	% properties (revisited). AVAILABLE AT: 
	% https://www.geometrictools.com/Documentation/PolyhedralMassProperties.pd
	%
	% VERIFICATION: this code yealds the same values as NMSBuilder for a femur
	% and a sphere. (Exactly the same values, but it's faster!)
	%
	...
	% ----------
	% v : __TYPE__
	% 	matrix [n x 3] vertices including the coordinates of n points
	% f : __TYPE__
	% 	matrix of [f x 3], collecting the indices of the vertices of
	% 	 each facet of the mes
	%
	% Returns
	% -------
	% MassProps : __TYPE__
	% 	Matlab Structure with Mass infos :
	% 	 * MassInfo.mass = mass;
	% 	 * MassInfo.COM = COM;
	% 	 * MassInfo.Imat = I; inertia matrix calculated at COM
	% 
