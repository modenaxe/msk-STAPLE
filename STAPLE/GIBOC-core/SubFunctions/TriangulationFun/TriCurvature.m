...
function [ Cmean, Cgaussian, Dir1, Dir2, Lambda1, Lambda2 ] = TriCurvature( TR, usethird, Rot0 )
    ...
    % Calculate Rotation matrices from the normals for each vertex
    M = zeros(3,3,nv);
    Minv = zeros(3,3,nv);
    for i = 1:nv 
    ...
    for i=1:nv
        ...
        % Neigbour Vertices of current iterarion vertex
        Ve = TR.Points(Nce,:);
    ...
    function [M, Minv] = VectorRotationMatrix(v)
        ...
        l = [k(2).*v(3)-k(3).*v(2), k(3).*v(1)-k(1).*v(3), k(1).*v(2)-k(2).*v(1)];
        l = l/sqrt(sum(l.^2));

        k = [l(2).*v(3)-l(3).*v(2), l(3).*v(1)-l(1).*v(3), l(1).*v(2)-l(2).*v(1)];
        k = k/sqrt(sum(k.^2));
