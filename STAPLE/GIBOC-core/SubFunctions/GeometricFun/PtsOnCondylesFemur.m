...
%
% [ PtsCondyle_end, PtsKeptID ] = PtsOnCondylesFemur( PtsCondyle_0 , 
%                     Pts_Epiphysis, CutAngle, In_Ratio, Out_Ratio )
%
%
...
%
% Inputs:
%   PtsCondyle_0 - Initial set of m certain condyle points formated as [mx3]
%                  matrix
%   Pts_Epiphysis - Set of n points that are candidate to be condyle points,
%                   formated as [nx3] matrix n>m
%   CutAngle - Angle in degree above which posterior points are considered as
%              not part of the condyles
%   In_Ratio - Relative distance to ellipse for inclusion of condyle points 
%              inside the ellipse.
%   Out_Ratio - Relative distance to the convex hull of the initial set of 
%               condyle points.
%
% Outputs:
%   PtsCondyle_end - all identified condyles points as a [lx3] matrix, l<n
%   PtsKeptID - Index of the identified condyle points relative to the matrix
%               of Pts_Epiphysis 
%
% -------------------------------------------------------------------------
...
%-------------------------------------------------------------------------%
function [ PtsCondyle_end, PtsKeptID ] = PtsOnCondylesFemur( PtsCondyle_0 , Pts_Epiphysis, CutAngle, In_Ratio, Out_Ratio )
    ...
    ellipse_x_r     = Elps.X0 + In_Ratio*Elps.a*cos( theta_r );
    ellipse_y_r     = Elps.Y0 + In_Ratio*Elps.b*sin( theta_r );
    ...
    % ConvexHull dilated by relative to the ellipse center distance
    IN_CH = inpolygon(Pts_Epiphysis(:,3),Pts_Epiphysis(:,1),...
        PtsCondyle_0(K,3)+Out_Ratio*(PtsCondyle_0(K,3)-Elps.X0_in),...
        PtsCondyle_0(K,1)+Out_Ratio*(PtsCondyle_0(K,1)-Elps.Y0_in));
