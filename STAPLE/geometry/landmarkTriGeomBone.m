function Landmarks = landmarkTriGeomBone(TriObj, CS, bone_name, results_plot)
...
for nL = 1:NL
    ...
    if strcmp(cur_land(end), 'proximal')
        local_BL = findLandmarkCoords(TriPoints(TriPoints(:,2)>ub,:), cur_land{2}, cur_land{3});
    elseif strcmp(cur_land(end), 'distal')
        local_BL = findLandmarkCoords(TriPoints(TriPoints(:,2)<lb,:), cur_land{2}, cur_land{3});
%         debug_plot = 1;
%         if debug_plot == 1
%             figure
...
%         end
        ...
        % get landmark
        local_BL = findLandmarkCoords(TriPoints, cur_land{2}, cur_land{3});
    end
