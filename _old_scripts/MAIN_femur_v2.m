...
patch('Faces',f,'Vertices',v_trasf); 
...
% Y>0
v_yPos = v_trasf(v_trasf(:,2)>0,:);
% lowest points

% Y>pos lat and dist
set1 = v_yPos(v_yPos(:,3)>0, :);
[~, ind1] = max(set1(:,3));
LFLE = set1(ind1,:);
plot_refsyst(gca, LFLE, eye(3), scale_BL);axis equal;grid on

[~, ind12] = max(set1(:,2));
LFMC = set1(ind12,:);
plot_refsyst(gca, LFMC, eye(3), scale_BL);axis equal;grid on

set2 = v_yPos(v_yPos(:,3)<0,:);
[~, ind2] = min(set2(:,3));
LFME = set2(ind2,:);
plot_refsyst(gca, LFME, eye(3), scale_BL);axis equal;grid on

[~, ind22] = max(set2(:,2));
LFLC = set2(ind22,:);
plot_refsyst(gca, LFLC, eye(3), scale_BL);axis equal;grid on
